...
总的来说，全局特种是对图像内容的高度抽象的概括。如果用户对整个图像的整体感兴趣，而不是对前景本身感兴趣的话，用全局特征来描述图像是比较合适的。但是无法分辨出前景和背景却是全局特征本身就有的劣势，特别是在关注的对象受到遮挡等影响的时候，全局特征很有可能就被破坏掉了。
...
基于像素级或block级的显著性检测方法中用到是center-surround contrast，而基于区域的方法用到的则是local contrast和global contrast，这是因为区域形状不规则，所以叫local contrast。
...
由于是先将图像分割成超像素，所以这里的分块就不是矩形，而是不规则区域，但计算是类似的，还是算该区域内所有像素点的方向梯度，然后对方向梯度直方图进行投票。

2）一定要将彩色图像先转化为灰度图像才能提取HOG特征吗？

HDCT~\cite{kim2014salient}方法中用到HOG特征~\cite{felzenszwalb2010object}
