...
问题：如果颜色均值是一种基于区域的特征，那么根据颜色均值求的对比度是特征吗？
...
论文Learning optimal seeds for diffusion-based salient object detection中用到

\subsection{Center Prior或Location Prior}

SDSP~\cite{zhang2013sdsp}中用到，并且定义如下：
\begin{align}
S_D(x) = exp(-\frac{||x-c||_2^2}{\sigma_D^2})
\end{align}
